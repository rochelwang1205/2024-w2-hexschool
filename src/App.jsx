...
function App() {
  ...
  const handleLogin = (e) => {  //神秘事件寫法
    ...
    .then((res) => {
      ...
      .catch((error) => alert('get products fail'))
      setIsAuth(true);
    })
  ...
  const checkUserLogin = () => {
    ...
    .then((res) => alert('user is login'))
    .catch((error) => console.error('user is not login'))
  }
  return (
    <>
    {isAuth ? <div className="container py-5">
      ...
      <form onSubmit={handleLogin} className="d-flex flex-column gap-3"> 
        {/* 通常神秘事件寫法的觸發不會寫在按鈕上(onClick)會寫在表單上 */}
